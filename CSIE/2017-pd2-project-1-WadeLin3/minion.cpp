...
static int TNK = 1; 
static int ATK = 3;
static int HEA = TNK + ATK;
static int mana_out = 0;
...
int best_card(int c1,int c2,int c3,int c4,int status,int chock){
	...
	if (c3 == 9) {return c3;}
	if (c4 == 9) {return c4;}
	if (c1 == 2 && chock == 1) {return c1;}
        if (c2 == 2 && chock == 1) {return c2;}
        if (c3 == 2 && chock == 1) {return c3;}
        if (c4 == 2 && chock == 1) {return c4;}
        if (c1 == 5) return c1;
        if (c2 == 5) return c2;
        if (c3 == 5) return c3;
        if (c4 == 5) return c4;
	if (tank_count >= TNK && attack_count > ATK) {tank_count = 0;attack_count = 0;}
	// count num of class
	...
	// special status
	switch (status){
		case 0: TNK= 1; ATK = 3; break; // normal status
		case 1: case 2: ATK = 2; break; // defend the main castle
		case 3: case 4: ATK = 5; break; // offensive
		case 5: case 6: ATK = 3; break; // normal defense
		case 7: case 8: ATK = 3; break; // normal attack
	}
	// normal status
	if (a!=0 && b!=0 && c!=0){
	...
	// no tank
                ...
                return find_card(c1,c2,c3,c4,2);
        }
	else {
		//cout<<"no found"<<endl;
		return 0;
...
int find_card(int c1,int c2,int c3,int c4, int cls){
	...
	if (cc[c1-1] == cls) best = c1;
        if (cc[c2-1] == cls) { 
		if (best != 0 && cm[best-1] <= cm[c2-1]) ;
		else best = c2;
	}
        if (cc[c3-1] == cls) { 
                if (best != 0 && cm[best-1] <= cm[c3-1]) ;
                else best = c3;
        }
        if (cc[c4-1] == cls) { 
                if (best != 0 && cm[best-1] <= cm[c4-1]) ;
                else best = c4;
        }
	return best;
}
...
int compare_attack_tank(){
	...
	else if ((attack_count%ATK) < ATK) return 0;
